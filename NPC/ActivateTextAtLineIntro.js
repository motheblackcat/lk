...
function Update() {
    ...
    if(waitForPress && Input.GetButtonDown("Up") || waitForPress && Input.GetAxis("Vertical") >= 1) {
        if (!hasBeenPressed) {
    ...
    if (textBoxManager.isTyping) {
        GameObject.Find("Bartender").GetComponent(Animator).SetBool("talk", true);
    }
    else {
        GameObject.Find("Bartender").GetComponent(Animator).SetBool("talk", false);
    }
}
